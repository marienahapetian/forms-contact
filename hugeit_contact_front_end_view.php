									...
									if($rowimages->hc_left_right == 'left'){
										...
										switch ($inputtype) {
											...
											case 'simple_captcha_box': //8.1
												?>
												<div class="hugeit-field-block simple-captcha-block" rel="huge-contact-field-<?=$rowimages->id; ?>" data-form_id="<?=$frontendformid; ?>" data-sitekey="<?=$paramssld['form_captcha_public_key']; ?>" data-theme="<?php echo $rowimages->hc_required; ?>" data-cname="<?php echo $rowimages->name; ?>">
													<?php if($rowimages->hc_input_show_default=='1'){$hg_left_right_class='formsLeftAlign';}
													else{$hg_left_right_class=$rowimages->hc_input_show_default;}?>

													<label class="formsAboveAlign">
														<img src="<?=plugin_dir_url(__FILE__);?>/admin/captcha.php?id=<?=$rowimages->id;?>&l=<?=$rowimages->hc_other_field;?>">
								...
								foreach ($rowim as $key=>$rowimages){
									if($rowimages->hc_left_right == 'right'){
										...
										switch ($inputtype) {
											...
											case 'simple_captcha_box': //8.1
												?>
												<div class="hugeit-field-block simple-captcha-block" rel="huge-contact-field-<?php echo $rowimages->id; ?>" data-form_id="<?php echo $frontendformid; ?>" data-sitekey="<?php echo $paramssld['form_captcha_public_key']; ?>" data-theme="<?php echo $rowimages->hc_required; ?>" data-cname="<?php echo $rowimages->name; ?>">
													<?php if($rowimages->hc_input_show_default=='1'){$hg_left_right_class='formsLeftAlign';}
													else{$hg_left_right_class=$rowimages->hc_input_show_default;}?>
      ...
      jQuery('#hugeit-contact-wrapper_<?php echo $frontendformid; ?>').find('.hugeit-field-block').not('.buttons-block').each(function(){
	 	...
	 	jQuery( "#huge_it_contact_form_<?php echo $frontendformid; ?>" ).on( "reset", function() {
			...
			function refresh_captcha() {
			    ...
			    newimg='<img src="<?=plugin_dir_url(__FILE__);?>/admin/captcha.php?id='+captchaid+'&l='+digits+'">';

				jQuery(newimg).prependTo(captchacontainer);

			}
			jQuery('#huge_it_contact_form_<?=$frontendformid;?> .captcha_refresh_button').click(refresh_captcha);
